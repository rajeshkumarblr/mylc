 ...
 *
 * Description:
 * You are given a 2D integer array  intervals , where  intervals[i] = [left i
 * , right i ]  describes the  i th   interval starting at  left i   and
 * ending at  right i    (inclusive) . The  size  of an interval is defined as
 * the number of integers it contains, or more formally  right i  - left i  +
 * 1 .
 *  You are also given an integer array  queries . The answer to the  j th
 * query is the  size of the smallest interval   i  such that  left i  <=
 * queries[j] <= right i  . If no such interval exists, the answer is  -1 .
