...
int main(int, char **) {
  ...
  string prob = "1";
  string category = "";
  if (const char *s = getenv("LC_PROB_NUM"))
    prob = s;
  if (const char *c = getenv("LC_CATEGORY"))
    category = c;
  ...
  const vector<int> solved = {1,   2,   3,   9,   11,  15,  20,  21,  33,
                              ...
                              238, 347, 424, 438, 560, 567, 739, 1235};
  ...
  map<int, TestFn> handlers = {
      ...
      {347, lc_test_347}, {1235, lc_test_1235}};

  auto print_header = []() {
    cout << left << setw(5) << "No" << setw(28) << "Description" << setw(16)
         << "Category" << setw(7) << "Result"
         << "Cases\n";
  };
  ...
  if (prob == "all" || !category.empty()) {
    ...
    for (int prob_num : solved) {
      ...
      string desc = test.contains("description")
                        ? test.at("description").get<string>()
                        : "";
      string cat =
          test.contains("category") ? test.at("category").get<string>() : "";
      ...
      if (test.contains("cases")) {
        ...
        for (int i = 1; i <= static_cast<int>(test.at("cases").size()); ++i)
    ...
    for (const auto &row : summary) {
      cout << left << setw(5) << row.num << setw(28)
           << (row.desc.size() > 27 ? row.desc.substr(0, 27) : row.desc)
           << setw(16) << row.cat << setw(7) << row.result << "[";
    ...
    cout << "\nSummary: problems=" << summary.size()
         << ", passed=" << passedProblems << ", failed=" << failedProblems
  ...
  string desc =
      test.contains("description") ? test.at("description").get<string>() : "";
  string cat =
      test.contains("category") ? test.at("category").get<string>() : "";
  ...
  print_header();
  cout << left << setw(5) << prob << setw(28)
       << (desc.size() > 27 ? desc.substr(0, 27) : desc) << setw(16) << cat
       << setw(7) << (ok ? "Pass" : "Fail") << "[";
  ...
  cout << "\nSummary: problems=1, passed=" << (ok ? 1 : 0)
       << ", failed=" << (ok ? 0 : 1) << ", total_cases=" << case_indices.size()
       << "\n";
