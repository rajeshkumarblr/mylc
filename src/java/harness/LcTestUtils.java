...
class ProblemsData {
    ...
    static ProblemsData loadFromFile() throws IOException {
        ...
        pd.problems = gson.fromJson(json, new com.google.gson.reflect.TypeToken<Map<String, ProblemTest>>() {
        }.getType());
        return pd;
    }
}
...
class Result {
    ...
    Result(List<Integer> i, boolean k) {
        indices = i;
        ok = k;
    }
}
...
class Registry {
    static Map<String, Problem> build() {
        ...
        m.put("56", cases -> Drivers.driver56(cases));
        ...
        m.put("33", cases -> Drivers.driver33(cases));
        ...
        m.put("13", cases -> Drivers.driver13(cases));
        ...
        m.put("981", cases -> Drivers.driver981(cases));
        m.put("359", cases -> Drivers.driver359(cases));
        m.put("362", cases -> Drivers.driver362(cases));
        m.put("199", cases -> Drivers.driver199(cases));
        m.put("76", cases -> Drivers.driver76(cases));
        return m;
    }
}
...
class Row {
    ...
    Row(int n, String d, String c, String r, List<Integer> cs) {
        num = n;
        desc = d;
        category = c;
        result = r;
...
class Drivers {
    ...
    private static int[] toIntArray(JsonArray arr) {
        ...
        for (int i = 0; i < arr.size(); i++)
            a[i] = arr.get(i).getAsInt();
        return a;
    }

    private static int[][] toInt2DArray(JsonArray arr) {
        if (arr == null)
            return new int[0][0];
        int[][] a = new int[arr.size()][];
        for (int i = 0; i < arr.size(); i++) {
            a[i] = toIntArray(arr.get(i).getAsJsonArray());
        }
        return a;
    }
    ...
    private static boolean deepEquals2D(List<List<Integer>> a, List<List<Integer>> b) {
        ...
        for (int i = 0; i < a.size(); i++) {
            ...
            for (int j = 0; j < ra.size(); j++)
                if (!Objects.equals(ra.get(j), rb.get(j)))
                    return false;
        }
        return true;
    }
    ...
    static Result driver11(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            int[] h = toIntArray(tc.getAsJsonArray("input"));
            int want = tc.get("expected").getAsInt();
    ...
    static Result driver42(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            int[] a = toIntArray(tc.getAsJsonArray("input"));
            int want = tc.get("expected").getAsInt();
    ...
    static Result driver100(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            TreeNode p = buildTree(tc.getAsJsonArray("p"));
            TreeNode q = buildTree(tc.getAsJsonArray("q"));
            boolean want = tc.get("expected").getAsBoolean();
    ...
    static Result driver199(List<JsonObject> cases) {
        List<Integer> idx = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            TreeNode root = buildTree(tc.getAsJsonArray("tree"));
            int[] want = toIntArray(tc.getAsJsonArray("expected"));
            List<Integer> gotList = new P199().rightSideView(root);
            int[] got = gotList != null ? gotList.stream().mapToInt(Integer::intValue).toArray() : new int[0];
            if (!Arrays.equals(got, want))
                okAll = false;
            idx.add(i + 1);
        }
    ...
    static Result driver567(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            String s1 = tc.get("s1").getAsString();
            String s2 = tc.get("s2").getAsString();
            boolean want = tc.get("expected").getAsBoolean();
    ...
    static Result driver739(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            int[] t = toIntArray(tc.getAsJsonArray("input"));
            int[] want = toIntArray(tc.getAsJsonArray("expected"));
    ...
    static Result driver198(List<JsonObject> cases) {
        List<Integer> idx = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
    ...
    static Result driver33(List<JsonObject> cases) {
        List<Integer> idx = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            ...
            int target = tc.get("target").getAsInt();
            int want = tc.get("expected").getAsInt();
            int got = new P33().new Solution().search(nums, target);
            if (got != want)
                okAll = false;
            idx.add(i + 1);
        }
    ...
    static Result driver36(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            char[][] board = to2DCharArray(tc.getAsJsonArray("board"));
            boolean want = tc.get("expected").getAsBoolean();
    ...
    static Result driver160(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            ...
            Integer want = tc.has("expected") && !tc.get("expected").isJsonNull() ? tc.get("expected").getAsInt()
                    : null;
    ...
    static Result driver200(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            char[][] grid = to2DCharArray(tc.getAsJsonArray("grid"));
            int want = tc.get("expected").getAsInt();
    ...
    static Result driver226(List<JsonObject> cases) {
        ...
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            TreeNode root = buildTree(tc.getAsJsonArray("root"));
            int[] want = toIntArray(tc.getAsJsonArray("expected"));
    ...
    static Result driver13(List<JsonObject> cases) {
        List<Integer> idx = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
    ...
    static Result driver350(List<JsonObject> cases) {
        List<Integer> idx = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            int[] nums1 = new Gson().fromJson(tc.get("nums1"), int[].class);
            int[] nums2 = new Gson().fromJson(tc.get("nums2"), int[].class);
            int[] want = new Gson().fromJson(tc.get("expected"), int[].class);
            int[] got = new P350().intersect(nums1, nums2);
            Arrays.sort(want);
            Arrays.sort(got);
    ...
    static Result driver242(List<JsonObject> cases) {
        List<Integer> indices = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            String s = tc.get("s").getAsString();
            String t = tc.get("t").getAsString();
            boolean want = tc.get("expected").getAsBoolean();
            boolean got = new P242().isAnagram(s, t);
            if (got != want) {
    ...
    static Result driver205(List<JsonObject> cases) {
        List<Integer> indices = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            ...
            String t = tc.get("t").getAsString();
            boolean want = tc.get("expected").getAsBoolean();
            boolean got = new P205().isIsomorphic(s, t);
            if (got != want) {
                System.out.printf("  Case %d: FAIL\n    Input: s=\"%s\", t=\"%s\"\n    Expected: %b, Got: %b\n", 
                    i + 1, s, t, want, got);
                okAll = false;
    ...
    static Result driver981(List<JsonObject> cases) {
        List<Integer> indices = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            JsonArray commands = tc.getAsJsonArray("commands");
            JsonArray args = tc.getAsJsonArray("args");
            JsonArray expected = tc.getAsJsonArray("expected");

            P981.TimeMap tm = null;
            boolean pass = true;
            for (int j = 0; j < commands.size(); j++) {
                String cmd = commands.get(j).getAsString();
                JsonArray arg = args.get(j).getAsJsonArray();
                
                if (cmd.equals("TimeMap")) {
                    tm = new P981.TimeMap();
                } else if (cmd.equals("set")) {
                    tm.set(arg.get(0).getAsString(), arg.get(1).getAsString(), arg.get(2).getAsInt());
                } else if (cmd.equals("get")) {
                    String got = tm.get(arg.get(0).getAsString(), arg.get(1).getAsInt());
                    String want = expected.get(j).isJsonNull() ? "" : expected.get(j).getAsString();
                    if (!got.equals(want)) {
                        System.out.printf("  Case %d, Op %d: FAIL\n    Cmd: %s, Args: %s\n    Expected: %s, Got: %s\n", 
                            i + 1, j, cmd, arg, want, got);
                        pass = false;
                        break;
                    }
                }
    ...
    static Result driver359(List<JsonObject> cases) {
        List<Integer> indices = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            ...
            for (int j = 0; j < commands.size(); j++) {
                ...
                if (cmd.equals("Logger")) {
                    obj = new P359();
                } else if (cmd.equals("shouldPrintMessage")) {
                    boolean got = obj.shouldPrintMessage(arg.get(0).getAsInt(), arg.get(1).getAsString());
                    boolean want = expected.get(j).getAsBoolean();
                    if (got != want) {
                        System.out.printf("  Case %d, Op %d: FAIL\n    Cmd: %s, Args: %s\n    Expected: %b, Got: %b\n",
                                i + 1, j, cmd, arg, want, got);
                        pass = false;
    ...
    static Result driver362(List<JsonObject> cases) {
        List<Integer> indices = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            ...
            for (int j = 0; j < commands.size(); j++) {
                ...
                if (cmd.equals("HitCounter")) {
                    obj = new P362();
                } else if (cmd.equals("hit")) {
                    obj.hit(arg.get(0).getAsInt());
                } else if (cmd.equals("getHits")) {
                    int got = obj.getHits(arg.get(0).getAsInt());
                    int want = expected.get(j).getAsInt();
                    if (got != want) {
                        System.out.printf("  Case %d, Op %d: FAIL\n    Cmd: %s, Args: %s\n    Expected: %d, Got: %d\n",
                                i + 1, j, cmd, arg, want, got);
                        pass = false;
    ...
    static Result driver56(List<JsonObject> cases) {
        List<Integer> indices = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            int[][] intervals = toInt2DArray(tc.getAsJsonArray("input"));
            int[][] want = toInt2DArray(tc.getAsJsonArray("expected"));
            int[][] got = new P56().merge(intervals);

            if (!Arrays.deepEquals(got, want)) {
                System.out.printf("  Case %d: FAIL\n    Expected: %s, Got: %s\n",
                        i + 1, Arrays.deepToString(want), Arrays.deepToString(got));
                okAll = false;
            }
            indices.add(i + 1);
        }
    ...
    static Result driver76(List<JsonObject> cases) {
        List<Integer> idx = new ArrayList<>();
        boolean okAll = true;
        for (int i = 0; i < cases.size(); i++) {
            JsonObject tc = cases.get(i);
            String s = tc.get("s").getAsString();
            String t = tc.get("t").getAsString();
            String want = tc.get("expected").getAsString();
            String got = new P76().new Solution().minWindow(s, t);
            if (!Objects.equals(got, want))
                okAll = false;
            idx.add(i + 1);
        }
